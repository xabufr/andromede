define(['./chat', './cursor', './progressBar', './objectscreentracker', './lockedTarget'], function(Chat, Cursor, ProgressBar, ObjectScreenTracker, LockedTarget) {
    'use strict';
    ...
    UiMain.prototype.update = function(Core, delta, localShip) {
        ...
        if(localShip) {
            ...
            if (localShip.target && !localShip.lockedTarget) {
                var lockedTraget = new LockedTarget();
                localShip.lockedTarget = this.createScreenTracker(localShip.target, lockedTraget);
            } else if (!localShip.target && localShip.lockedTarget) {
                this.deleteScreenTracker(localShip.lockedTarget);
                localShip.lockedTarget = null;
            }
        }
